...
class KeyEditorWindow(ctk.CTkToplevel):
    ...
    def __init__(
        ...
        # Mỗi phần tử: {"dap_an": CTkTextbox, "diem": CTkEntry, "frame": CTkFrame}
    ...
    def _build_tl_tab(self, tab):
        ...
        ctk.CTkLabel(col_hdr, text="Đáp án mẫu (text so sánh)",
                     font=ctk.CTkFont(size=12, weight="bold"),
                     text_color="gray70").grid(row=0, column=1, padx=4, pady=6, sticky="w")
        ctk.CTkLabel(col_hdr, text="Điểm tối đa", width=100,
                     font=ctk.CTkFont(size=12, weight="bold"),
                     text_color="gray70").grid(row=0, column=2, padx=(4,14), pady=6)
    ...
    def _add_tl_row(self, q_num: int):
        ...
        tb.grid(row=row, column=1, padx=6, pady=4, sticky="ew")
        tb.insert("0.0", "")
        ...
        diem_entry = ctk.CTkEntry(
            ...
            width=90, height=36, justify="center",
            font=ctk.CTkFont(size=13, weight="bold"),
            placeholder_text="điểm")
        diem_entry.grid(row=row, column=2, padx=(4,10), pady=4)
        diem_var.trace_add("write", lambda *_: self._update_tl_total())

        self._tl_rows.append({"tb": tb, "diem_var": diem_var})
    ...
    def _get_tl_list(self) -> list[dict]:
        ...
        for i, r in enumerate(self._tl_rows):
            ...
            try:
                ...
                diem = 0.0
            result.append({"q": i + 1, "dap_an": dap_an, "diem": diem})
        return result
